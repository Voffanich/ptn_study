...
def save_excel(table, table_f):
    ...
    """
    writer = pd.ExcelWriter('C:\\Users\\Matushev_work\\Desktop\\СОХРАН\\bazis\\output_fm.xlsx', engine='xlsxwriter')
    writer_f = pd.ExcelWriter('C:\\Users\\Matushev_work\\Desktop\\СОХРАН\\bazis\\output_f_fm.xlsx', engine='xlsxwriter')
    ...
    for column in table:
        col_width = max(table[column].astype(str).map(len).max(), len(column))*1.05
    ...
    fig_fmt_f = workbook_f.add_format({'num_format':'0,0', 'align':'center', 'bold':False})    
